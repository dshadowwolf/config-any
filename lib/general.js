...
var block = function(blockName) {
	...
	this.cbStore[childBlock.id] = childBlock;
    };
    this.addData = function(keyname,value) {
	if( this.dataStore[keyname] ) {
	    if( !Array.isArray(this.dataStore[keyname]) ) {
		var tdsi = this.dataStore[keyname];
		if( typeof tdsi === 'number' ) {
		    tdsi = tdsi.toString();
		}
		this.dataStore[keyname] = JSON.parse("[\""+tdsi+"\"]");
	    }
	    this.dataStore[keyname].push(value);
	} else {
  ...
  if( blockStore.dataStore != [] ) {
    for( var x in blockStore.dataStore ) {
      if( lcn ) {
        var t = ""+x;
        var k = t.toLowerCase();
        myData[k] = blockStore.dataStore[x];
      } else
        myData[x] = blockStore.dataStore[x];
    }
  }
  ...
  if( blockStore.cbStore != [] ) {
    for( var x in blockStore.cbStore ) {
      if( lcn ) {
        var t = ""+x;
        var k = t.toLowerCase();
        myData[k] = createData(blockStore.dataStore[x]);
      } else
